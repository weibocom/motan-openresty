...
function _M.serialize(params)
            ...
            if type(v) ~= "table" then
                ...
                btemp =
                    ...
                    utils.msb_numbertobytes(#k, consts.MOTAN_DATA_PACK_INT32_BYTE) ..
                        k .. utils.msb_numbertobytes(#v, consts.MOTAN_DATA_PACK_INT32_BYTE) .. v
    ...
    elseif p_type == "string" then
        buffer =
            utils.msb_numbertobytes(1, consts.MOTAN_SIMPLE_TYPE_BYTE) ..
...
function _M.deserialize(data)
    ...
    elseif buf_type == 1 then
        ...
        ngx.log(ngx.NOTICE, "deserialized string lenth:" .. body_len)
    elseif buf_type == 2 then
        ...
        while key ~= "" do
            value_len =
                utils.msb_stringtonumber(str_sub(map_buf, map_pos, map_pos + consts.MOTAN_DATA_PACK_INT32_BYTE - 1))
            map_pos = map_pos + consts.MOTAN_DATA_PACK_INT32_BYTE
