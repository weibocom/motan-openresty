...
function _M.buildHeader(self, msg_type, proxy, serialize, request_id, msg_status)
    ...
    -- local status = 0x08 | (msg_status & 0x07)
    local status = bor(0x08, band(msg_status, 0x07))
    -- local serial = 0x00 | (serialize << 3)
...
function _M.buildRequestHeader(self, request_id)
    return self:buildHeader(consts.MOTAN_MSG_TYPE_REQUEST, true, consts.MOTAN_SERIALIZE_SIMPLE, request_id, consts.MOTAN_MSG_STATUS_NORMAL)
end
